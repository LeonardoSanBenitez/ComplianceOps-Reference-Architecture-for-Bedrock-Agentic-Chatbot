...
- **Handler**: `app/handler.py` — Lambda entry point; parses request, invokes agent, returns JSON with EU AI Act Art. 50 disclosure headers

**EU AI Act Art. 50(1) compliance**: every response includes an `X-AI-Generated: true` header and a disclosure in the response body. The system prompt instructs the agent to identify itself as an AI on request.
